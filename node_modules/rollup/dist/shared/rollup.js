...
function assignChunksToBundle(chunks, outputBundle) {
    for (const chunk of chunks) {
        const chunkdDescription = (outputBundle[chunk.id] = chunk.getPrerenderedChunk());
...
function deconflictChunk(modules, dependencies, imports, usedNames, format, interop, preserveModules, syntheticExports, exportNamesByVariable) {
    for (const module of modules) {
...
function escapeId(id) {
    return id.replace(quoteNewlineRegEx, match => replacements[match]);
}
...
class Chunk$1 {
        ...
        for (const [module, code] of this.renderedModuleSources) {
            for (const { node, resolution } of module.dynamicImports) {
                ...
                node.renderFinalResolution(code, renderedResolution, resolution instanceof Module &&
                    ...
                    resolution.chunk.exportNamesByVariable.get(resolution.namespace)[0], options);
            }
        }
    }
    ...
    setIdentifierRenderResolutions(options) {
        ...
        deconflictChunk(this.orderedModules, this.dependencies, this.imports, usedNames, options.format, options.interop, this.inputOptions.preserveModules, syntheticExports, this.exportNamesByVariable);
    }
...
class ModuleLoader {
    ...
    async addManualChunks(manualChunks) {
        ...
        const result = this.extendLoadModulesPromise(Promise.all(unresolvedManualChunks.map(({ id }) => this.loadEntryModule(id, false, undefined, null))).then(manualChunkModules => {
...
async function rollupInternal(rawInputOptions, watcher) {
    const { options: inputOptions, unsetOptions } = getInputOptions(rawInputOptions, watcher !== null);
